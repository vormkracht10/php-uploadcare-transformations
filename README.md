...
Control who and for how long can access files in your project via Signed URLs. You will need to contact Uploadcare support to enable this feature as stated in the documentation [here](https://uploadcare.com/docs/security/secure-delivery/#authenticated-urls).

```php
$uuid = '12a3456b-c789-1234-1de2-3cfa83096e25';
$cdnUrl = 'https://example.com/cdn/';

$url = uc($uuid, $cdnUrl, `{timestamp`, `{acl}`, `{token}`)
        ->crop(width: 320, height: '50p', offsetX: 'center')
        ->setFill(color: 'ffffff')
        ->getSignedUrl();

echo $url;
// https://example.com/cdn/12a3456b-c789-1234-1de2-3cfa83096e25/-/crop/320x50p/center/-/set_fill/ffffff/?token=exp={timestamp}~acl={acl}~hmac={token}
```
